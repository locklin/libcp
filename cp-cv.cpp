...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < prob->num_ex; ++i) {
    ...
    output_file << std::resetiosflags(std::ios::fixed) << prob->y[i] << ' ' << predict_labels[i][0] << ' '
                << std::setiosflags(std::ios::fixed) << conf[i] << ' ' << cred[i];
    if (predict_labels[i][0] == prob->y[i]) {
      ++num_correct;
    }
    ...
    if (predict_labels[i].size() == 1) {
      ...
      for (size_t j = 1; j < predict_labels[i].size(); ++j) {
        ...
        if (predict_labels[i][j] == prob->y[i]) {
          ++num_incl;
        }
      }
  ...
  avg_conf /= prob->num_ex;
  avg_cred /= prob->num_ex;
  ...
  std::cout << "Accuracy: " << 100.0*num_incl/prob->num_ex << '%'
            << " (" << num_incl << '/' << prob->num_ex << ") "
            << "Multi Prediction: " << std::fixed << std::setprecision(4) << 100.0*num_multi/prob->num_ex << "%, "
            << "Empty Prediction: " << 100.0*num_empty/prob->num_ex << "%\n";
  output_file.close();
...
void ExitWithHelp() {
  std::cout << "Usage: cp-cv [options] data_file [output_file]\n"
            << "options:\n"
